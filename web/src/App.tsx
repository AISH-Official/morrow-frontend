...
import{ApiRequestError,addPersonalMemory,clearAssistantConversation,clearWellnessData,createCheckIn,deleteAccountCompletely,exportAccountData,getAiHealthConsent,getConnectedDevices,getDashboard,getPersonalization,getStoredWebSession,getWeeklyReport,isSessionExpired,loginWebSession,logoutWebSession,pairWebSession,rebuildPersonalization,revokeConnectedDevice,sendAssistantMessage,setAiHealthConsent,submitRecommendationFeedback}from'./api';
...
export default function App(){
 ...
 return <div className={`app-shell phase-${phase}`}>
  ...
  <main className="main-area">
   ...
   {view==='privacy'&&<PrivacyView mode={mode} account={account} profile={personalization} memories={memories} devices={devices} aiHealthConsent={aiHealthConsent} onAIHealthConsent={value=>void changeAIHealthConsent(value)} onPair={pairAccount} onRevoke={id=>void revokeDevice(id)} onRebuild={()=>void rebuildLearning()} onAdd={addMemory} onExport={()=>void downloadData()} onForget={()=>void forgetConversation()} onDelete={()=>void deleteData()} onDeleteAccount={()=>void deleteAccount()} onLogout={()=>void logout()}/>}
...
function ReportView({report}:{report:WeeklyReport}){
 ...
 return <div className="page-view report-view"><div className="page-heading"><div><span>WEEKLY PATTERN REPORT</span><h1>이번 주, 무엇이 달랐을까요?</h1><p>단정 대신 반복된 신호와 회복에 도움이 된 행동을 보여줘요.</p></div><div className="week-chip">{weekRange()}</div></div><div className="report-grid"><section className="report-hero"><div><span>WELLNESS MOMENTUM</span><b>{report.totalCheckIns?Math.round(report.improvementRate):'—'}</b><small>{report.totalCheckIns?`지난주 대비 ${report.changeFromPrevious>=0?'+':''}${Math.round(report.changeFromPrevious)}점`:'체크인이 필요해요'}</small></div><div className="weekly-chart">{points.map((point,index)=><div key={point.date}><i style={{height:`${point.score??2}%`}} className={point.score!==null&&point.score===Math.max(...points.map(value=>value.score??0))?'peak':''}/><span>{['일','월','화','수','목','금','토'][new Date(`${point.date}T00:00:00`).getDay()]}</span></div>)}</div></section><section className="report-summary"><span>이번 주 요약</span><h2>{report.insights}</h2><div><small>체크인</small><b>{report.totalCheckIns}회</b></div><div><small>가장 잦은 상태</small><b>{statusLabel[report.topStatus??'']??'기록 없음'}</b></div><div><small>주요 맥락</small><b>{causeLabel[report.topCause??'']??'기록 없음'}</b></div></section><section className="patterns"><span>PATTERN INTELLIGENCE</span>{report.patterns.map((pattern,index)=><article key={pattern}><i>0{index+1}</i><p>{translatePattern(pattern)}</p></article>)}</section><section className="trust-card"><ShieldCheck/><div><b>이 인사이트를 믿을 수 있는 이유</b><p>실제 일별 체크인과 지난주 기록만 비교했어요. 의료적 판단이나 다른 사용자와의 비교는 하지 않습니다.</p></div></section></div></div>
}

function PrivacyView({mode,account,profile,memories,devices,aiHealthConsent,onAIHealthConsent,onPair,onRevoke,onRebuild,onAdd,onExport,onForget,onDelete,onDeleteAccount,onLogout}:{mode:ConnectionMode;account:WebSession|null;profile:PersonalizationProfile|null;memories:UserMemory[];devices:ConnectedDevice[];aiHealthConsent:boolean;onAIHealthConsent:(value:boolean)=>void;onPair:(code:string)=>Promise<void>;onRevoke:(id:string)=>void;onRebuild:()=>void;onAdd:(type:'PREFERENCE'|'GOAL',summary:string)=>Promise<void>;onExport:()=>void;onForget:()=>void;onDelete:()=>void;onDeleteAccount:()=>void;onLogout:()=>void}){
 const[memoryText,setMemoryText]=useState('');const[memoryType,setMemoryType]=useState<'PREFERENCE'|'GOAL'>('PREFERENCE');const[saving,setSaving]=useState(false);const[pairCode,setPairCode]=useState('');const[pairing,setPairing]=useState(false);const[pairError,setPairError]=useState('');
 ...
 return <div className="page-view privacy-view"><div className="page-heading"><div><span>PRIVACY & PERSONAL MEMORY</span><h1>AI가 무엇을 기억하는지 직접 확인하세요</h1><p>범용 모델을 재학습하지 않고, 이 계정의 근거 있는 패턴만 개인화 컨텍스트로 사용해요.</p></div><button className="page-action" onClick={onRebuild}><RefreshCw/> 기록에서 다시 학습</button></div>
  <section className="device-pairing"><div><Watch/><span>DEVICE ACCOUNT</span><b>{account?`사용자 ${account.userId}`:'서버 연결 대기'}</b><p>iPhone 설정의 코드를 여기서 한 번 연결하면 이 아이디에 저장됩니다. 이후 다시 로그인해도 Watch·iPhone 연결이 유지됩니다.</p></div><form onSubmit={submitPair}><input aria-label="iPhone 연결 코드" maxLength={8} value={pairCode} onChange={event=>setPairCode(event.target.value.toUpperCase().replace(/[^2-9A-HJ-NP-Z-]/g,''))} placeholder="6자리 연결 코드"/><button disabled={pairing||!pairCode.trim()}>{pairing?<RefreshCw className="rotating"/>:<Signal/>} {pairing?'최신 데이터 확인 중':'이 아이디에 한 번 연결'}</button>{pairError?<small className="pair-error">{pairError}</small>:<small>이 웹의 추가 기기 코드: {account?.pairingCode??'—'}</small>}</form></section>
  <section className="connected-devices"><span>CONNECTED DEVICES</span>{devices.map(device=><article key={device.id}><div><b>{device.deviceName}</b><small>{device.platform} · {new Date(device.lastSeenAt).toLocaleString('ko-KR')}</small></div>{device.deviceId!==account?.deviceId&&<button onClick={()=>onRevoke(device.id)}>연결 해제</button>}</article>)}</section>
  <section className="ai-consent"><div><BrainCircuit/><span><b>AI 답변에 건강 요약 사용</b><small>직접 동의한 경우에만 파생 건강 요약을 AI 컨텍스트에 포함합니다.</small></span></div><button className={aiHealthConsent?'on':''} onClick={()=>onAIHealthConsent(!aiHealthConsent)}>{aiHealthConsent?'허용됨':'허용 안 함'}</button></section>
