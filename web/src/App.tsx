...
export default function App(){
 ...
 async function feedback(helpful:boolean,completed=true){
  if(!dashboard?.recommendation)return;
  try{await submitRecommendationFeedback(dashboard.recommendation.id,helpful,completed);const personal=await getPersonalization();setPersonalization(personal.profile);setMemories(personal.memories)}catch(error){if(isSessionExpired(error))expireSession();else{setMode('offline');setToast('피드백을 저장하지 못했습니다.');}return}
  if(!completed){setFeedbackDone(true);setToast('다음에는 다른 회복 방법을 제안할게요.');return}
 ...
 async function authenticate(accountId:string,password:string,signup:boolean){const value=signup?await signupWebSession(accountId,password):await loginWebSession(accountId,password);setAccount(value);hydrateAccount(value);setLoginMessage('');setLoginRequired(false);await loadData()}
 ...
 if(loginRequired)return <WebLoginView onAuthenticate={authenticate} message={loginMessage}/>;
...
function HealthDetailCard({dashboard}:{dashboard:Dashboard}){
 const{sleep,workouts}=dashboard.healthDetails??{sleep:null,workouts:[]};if(!sleep&&!workouts.length)return null;
 ...
 const intensity=(value:string)=>value==='HIGH'?'높은 강도':value==='MODERATE'?'중간 강도':'가벼운 강도';
 return <div className="health-details"><span>HEALTHKIT DETAILS · WATCH + IPHONE</span>{sleep&&<div><MoonStar/><p><b>수면 {time(sleep.startAt)}–{time(sleep.endAt)}</b><small>코어 {sleep.coreMinutes}분 · 깊은 {sleep.deepMinutes}분 · REM {sleep.remMinutes}분 · 깨어있음 {sleep.awakeMinutes}분</small></p></div>}{workouts.slice(0,3).map(workout=><div key={workout.clientWorkoutId}><Activity/><p><b>{workout.activityType} · {Math.round(workout.durationMinutes)}분 · {intensity(workout.intensity)}</b><small>{time(workout.startAt)}–{time(workout.endAt)} · 평균 {Math.round(workout.averageHeartRate)} / 최대 {Math.round(workout.maxHeartRate)} bpm · {Math.round(workout.activeEnergyKcal)} kcal</small></p></div>)}<em>강도는 심박과 활동 에너지 기반 참고값</em></div>
}
...
function RecoveryModal({action,title,durationSeconds,reason,confidence,onClose,onComplete}:{action:RecoveryAction;title?:string;durationSeconds:number;reason:string;confidence:string;onClose:()=>void;onComplete:(outcome:'IMPROVED'|'SAME'|'WORSE')=>Promise<void>}){
 const totalDuration=Math.max(30,durationSeconds);const[remaining,setRemaining]=useState(totalDuration);const[running,setRunning]=useState(false);const[attemptId,setAttemptId]=useState('');const[starting,setStarting]=useState(false);const[submitting,setSubmitting]=useState(false);const[saveError,setSaveError]=useState('');
 ...
 async function toggleRunning(){if(running){setRunning(false);return}if(attemptId){setRunning(true);return}if(starting)return;setStarting(true);setSaveError('');try{const value=await createRecoveryAttempt(action,reason,confidence);setAttemptId(value.id);setRunning(true)}catch{setSaveError('실행 기록을 시작하지 못했어요. 연결을 확인하고 다시 시도해 주세요.')}finally{setStarting(false)}}
 async function finish(outcome:'IMPROVED'|'SAME'|'WORSE'){if(submitting)return;if(!attemptId){setSaveError('실행 기록이 없어 완료할 수 없어요. 다시 시작해 주세요.');return}setSubmitting(true);setSaveError('');try{await completeRecoveryAttempt(attemptId,outcome);await onComplete(outcome)}catch{setSaveError('완료 기록을 저장하지 못했어요. 연결을 확인하고 다시 시도해 주세요.')}finally{setSubmitting(false)}}
 return <div className="modal-backdrop" onMouseDown={event=>{if(event.target===event.currentTarget)onClose()}}><section className="recovery-modal"><button className="modal-close" onClick={onClose} aria-label="닫기"><X/></button><span>WHY NOW · {confidence}</span><p className="recovery-reason">{reason}</p><div className="recovery-timer" style={{'--progress':`${((totalDuration-remaining)/totalDuration)*360}deg`} as React.CSSProperties}><div><b>{countdown(remaining)}</b><small>{running?'회복 중':durationLabel(totalDuration)}</small></div></div>{remaining>0?<><h2>{title??labels[action][0]}</h2><p>{labels[action][1]}</p><button className="recovery-start" disabled={starting} onClick={()=>void toggleRunning()}>{starting?'기록 준비 중':running?'잠시 멈춤':'지금 시작'}</button></>:<><h2>실행 전보다 조금 나아졌나요?</h2><div className="recovery-outcomes"><button disabled={submitting} onClick={()=>void finish('IMPROVED')}>나아졌어요</button><button disabled={submitting} onClick={()=>void finish('SAME')}>그대로예요</button><button disabled={submitting} onClick={()=>void finish('WORSE')}>더 불편해요</button></div></>}{saveError&&<p className="recovery-error" role="alert">{saveError}</p>}</section></div>
}
...
function CheckInModal({userId,onClose,onSave}:{userId:string;onClose:()=>void;onSave:(input:CheckInInput)=>Promise<void>}){
 ...
 async function submit(event:FormEvent){event.preventDefault();if(saving)return;setSaving(true);await onSave({userId,clientEventId:eventId.current,status,cause,note:note.trim(),source:'WEB',recordedAt:new Date().toISOString()});setSaving(false)}
...
function WebLoginView({onAuthenticate,message}:{onAuthenticate:(accountId:string,password:string,signup:boolean)=>Promise<void>;message:string}){
 const[signup,setSignup]=useState(false);const[accountId,setAccountId]=useState('');const[password,setPassword]=useState('');const[confirmation,setConfirmation]=useState('');const[loading,setLoading]=useState(false);const[error,setError]=useState('');
 function changeMode(value:boolean){setSignup(value);setPassword('');setConfirmation('');setError('')}
 async function submit(event:FormEvent){event.preventDefault();const clean=accountId.trim();if(clean.length<2){setError('아이디는 2자 이상 입력해 주세요.');return}if(password.length<8){setError('비밀번호는 8자 이상 입력해 주세요.');return}if(signup&&password!==confirmation){setError('비밀번호 확인이 일치하지 않습니다.');return}setLoading(true);setError('');try{await onAuthenticate(clean,password,signup)}catch(reason){if(reason instanceof ApiRequestError&&reason.status===409)setError('이미 사용 중인 아이디입니다.');else if(reason instanceof ApiRequestError&&reason.status===401)setError('아이디 또는 비밀번호를 확인해 주세요.');else setError('잠시 후 다시 시도해 주세요.')}finally{setLoading(false)}}
 return <main className="login-shell"><div className="login-glow"/><section className="login-card"><div className="login-brand"><i>M</i><div><b>MORROW</b><span>{signup?'CREATE ACCOUNT':'ACCOUNT LOGIN'}</span></div></div><div className="auth-mode-switch"><button type="button" className={!signup?'active':''} onClick={()=>changeMode(false)}>로그인</button><button type="button" className={signup?'active':''} onClick={()=>changeMode(true)}>회원가입</button></div><div className="login-copy"><span>ONE ACCOUNT · ALL DEVICES</span><h1>{signup?'나만의 회복 기록을\n시작하세요':'내 계정으로\n다시 연결하세요'}</h1><p>{signup?'아이디와 비밀번호만 만들면 사용자별 건강 기록이 분리됩니다.':'기존 테스트 계정을 포함해 가입한 계정으로 로그인할 수 있습니다.'}</p></div>{message&&<p className="login-notice">{message}</p>}<form onSubmit={submit}><label><span>아이디</span><input autoFocus autoComplete="username" minLength={2} maxLength={80} value={accountId} onChange={event=>setAccountId(event.target.value)} placeholder="한글 또는 영문 아이디"/></label><label><span>비밀번호</span><input type="password" autoComplete={signup?'new-password':'current-password'} minLength={8} maxLength={120} value={password} onChange={event=>setPassword(event.target.value)} placeholder="8자 이상"/></label>{signup&&<label><span>비밀번호 확인</span><input type="password" autoComplete="new-password" minLength={8} maxLength={120} value={confirmation} onChange={event=>setConfirmation(event.target.value)} placeholder="한 번 더 입력"/></label>}{error&&<p className="login-error">{error}</p>}<button disabled={loading||accountId.trim().length<2||password.length<8||(signup&&confirmation.length<8)}>{loading?<RefreshCw className="rotating"/>:<LockKeyhole/>}{loading?'계정 확인 중':signup?'계정 만들기':'로그인'}</button></form><footer><ShieldCheck/> 테스트 계정은 그대로 유지되며 새 계정 데이터와 분리됩니다.</footer></section></main>
}
